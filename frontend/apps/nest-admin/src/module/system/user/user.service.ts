...
export class UserService {
  ...
  /**
   * 登陆
   * TODO:登陆管理：第一次实现：我们先不用jwt模块，捋顺整个流程：这里createToken写死生成的token：Admin-Token
   * 这里对应前端的getToken函数那里，也是写死：获取这个
   * jwt后面的模块引入先不引入，所以这里就是普通假写死一个token，前端传这个假的。
   * 1.首先：账号登陆，co能够数据库查找字段userId的字段，相关用户名和密码
   * 2.接着：createToken根据uuid和userId生成token
   * 注意：这里uuid生成策略：
   * 3.接下来：todo：根据各种信息，从redis设置保存：刚刚登陆用户的信息
   * 4.
   *TODO:登陆管理：第二次完善：引入jwt真正的生成：
  ...
  async login(user: LoginDto, clientInfo: ClientInfoDto) {
    ...
    debugger;
    // todo：根据各种信息，从redis设置保存：刚刚登陆用户的信息
  ...
  createToken(payload: { uuid: string; userId: number }): string {
    // 注意：这里sign和signAsync分别代表同步和异步，异步获取在配置数据库中常见，返回promise，常常使用
    // await等待这个异步操作，确保从数据库中获取
    //  NestJS 中，生成 JWT 的规则是基于传入的 payload 和配置 JwtModule 的策略来决定的。 payload 通常包含一些用户信息或者其他需要在 token 中存储的数据。JwtModule 的配置则会决定 token 的签名、加密算法、有效期等参数。
