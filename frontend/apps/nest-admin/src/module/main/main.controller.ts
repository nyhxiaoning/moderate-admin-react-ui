...
export class MainController {
  ...
  async getCode(@Request() req) {
    ...
    /**
     * TODO: 这里需要对接具体的验证码服务
     * 字段	含义 secretKey	验证码交互加密使用的密钥，前端提交验证结果时需要加密坐标。
     * originalImageBase64	验证码原图的Base64编码
     * point	验证码坐标
     * jigsawImageBase64	拼图验证码图片的Base64编码
     * wordList	文字验证码内容
     * pointList	点选验证码坐标集合
     * pointJson	点选验证码坐标集合的JSON字符串格式
     * token	验证码令牌，有效期5分钟
     * result	验证结果，true成功，false失败
     * captchaVerification	前端提交的验证结果，供后端验证使用
     * clientUid	客户端唯一标识
     * ts	时间戳，精确到毫秒
     * browserInfo	浏览器信息
     * repCode	响应码
  ...
  @Get('/auth/get-permission-info')
  getPermissionInfo(@Request() req) {
    const user = req.user;
    return {
      code: 0,
      data: {
  ...
  @ApiOperation({
    summary: '用户信息：用户信息写死的数据库，目前没有写入',
  })
  @Get('/getInfo')
  async getInfo(@Request() req) {
    ...
    return {
      ...
      user: user?.user || {
        ...
        delFlag: '0',
        createBy: 'admin',
        createTime: '2024-05-07T11:56:52.000Z',
        updateBy: '',
        updateTime: null,
        remark: '管理员',
        userId: '1',
        ...
        password: '123456',
        loginIp: '127.0.0.1',
        loginDate: '2024-05-07T11:56:52.000Z',
        dept: {
          status: '0',
          delFlag: '0',
          createBy: 'admin',
