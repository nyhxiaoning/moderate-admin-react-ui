...
// 请求白名单，无须 token 的接口
// TODO:请求白名单，无须 token 的接口
// TODO:请求白名单，无须 token 的接口
// TODO:请求白名单，无须 token 的接口
// TODO:请求白名单，无须 token 的接口
// TODO:请求白名单，无须 token 的接口: 增加：/system/menu
  ...
  async (response: AxiosResponse<any>) => {
    ...
    if (ignoreMsgs.indexOf(msg) !== -1) {
      ...
      if (!isRefreshToken) {
        ...
        // 2. 进行刷新访问令牌
        try {
          const { payload } = await dpChain("authStore").refreshTokenAct(null);
          dpChain("authStore").setToken(payload);
