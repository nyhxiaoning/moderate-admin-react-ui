...
export class AppHelper extends HelperBase {
  ...
  createMenuDataLoopEx(params: {
    ...
    const clientChildLoop = (severMenuItem: MenuItem) => {
      ...
      for (let key in routesMap) {
        ...
        if (clientMenuItem.parentId == severMenuItem.key && !isRegisterServer) {
  ...
  getMenuConfigByPathNameEx(pathName: string, menuList: MenuItemData[]) {
    ...
    const routeDataItem = this.routerHelper.getRoutItemConfigByPath(pathName);
    if (routeDataItem) {
      const loop = (routeDataItem: RouteItem) => {
        ...
        if (!menuDataItem) {
          if (routeDataItem.depends?.length) {
            const parentRouteDataItem = this.routerHelper.getRoutItemConfigById(
  ...
  getMenuConfigByPathName(pathName: string) {
    ...
    selectedKeys.forEach((selectKey) => {
      for (let key in routesMap) {
        ...
        // 判断一下，
        if (item.path == pathName && item.parentId == selectKey && item.index) {
      ...
      if (activeTabKey == pathName) {
        ...
        itemIndex != -1 &&
          this.routerHelper.jumpToByPath(
            tabItems[
              itemIndex == tabItems.length - 1 ? itemIndex - 1 : itemIndex + 1
            ]?.key,
          );
      }
    }
  ...
  closeRightTabByPath({ pathName }: { pathName?: string } = {}) {
    ...
    this.dpChain("appStore").setTabItems(tabItems.slice(0, targetIndex + 1));
  }
    ...
    if (!isTab && depends?.length) {
      const targetItem = tabItems.find((item) => {
        const parentIndexRouteId =
          this.routerHelper.getIndexRoute(depends[0])?.id || depends[0];
        return (
          item.key.toLowerCase() ==
          this.routerHelper.getRoutePathByKey(parentIndexRouteId).toLowerCase()
        );
      });
